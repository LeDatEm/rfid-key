...
FirebaseData stream_add;
FirebaseData stream_del;
FirebaseData fbdo;
...
boolean add_web = false;
boolean del_web = false;
boolean match = false;   
...
void config_FIREBASE(){
  ...
  stream_add.setBSSLBufferSize(2048 /* Rx buffer size in bytes from 512 - 16384 */, 1024 /* Tx buffer size in bytes from 512 - 16384 */);
  stream_del.setBSSLBufferSize(2048 /* Rx buffer size in bytes from 512 - 16384 */, 1024 /* Tx buffer size in bytes from 512 - 16384 */);
  Firebase.begin(&config, &auth);
  Firebase.reconnectWiFi(true);
  stream_add.keepAlive(5, 5, 1);
  stream_del.keepAlive(5, 5, 1);
  if (!Firebase.RTDB.beginStream(&stream_add, "rfid/master/add/card/cardUID"))
    Serial.printf("sream begin error, %s\n\n", stream_add.errorReason().c_str());

  Firebase.RTDB.setStreamCallback(&stream_add, streamCallback_ADD, nullptr);

  if (!Firebase.RTDB.beginStream(&stream_del, "rfid/master/delete/card/cardUID"))
    Serial.printf("sream begin error, %s\n\n", stream_del.errorReason().c_str());

  Firebase.RTDB.setStreamCallback(&stream_del, streamCallback_DEL, nullptr);
...
void deleteID( byte a[] ) {
  if ( !findID( a ) ) {     // Before we delete from the EEPROM, check to see if we have this card!
    ...
    EEPROM.commit();
    successDelete();
    Serial.println(F("ID removido da EEPROM com successfully"));
  }
}
  ...
  if (customKey){
    ...
    if (count == 5) {
      ...
      if (strncmp(enteredPassword, password1, 5) == 0) {
        ...
        ws2812fx.show();
        if (Firebase.RTDB.setInt(&fbdo, "rfid/user/keypad/pass/anonymous", random(1,100))) {
          Serial.println("Card UID sent to Firebase successfully");
        } 
        //else {
        //   Serial.println("Failed to send Card UID to Firebase");
        ...
        if (!secondPasswordEntered) {
          ...
          ws2812fx.setPixelColor(0, PINK);
          ws2812fx.show();
          if (Firebase.RTDB.setString(&fbdo, "rfid/master/password", "*101#")) {
            Serial.println("Card UID sent to Firebase successfully");
          }
          //  else {
          //   Serial.println("Failed to send Card UID to Firebase");
...
void streamCallback_ADD(FirebaseStream add_card)
{
  // Serial.printf("sream path, %s\nevent path, %s\ndata type, %s\nevent type, %s\n\n",
  //               add_card.streamPath().c_str(),
  //               add_card.dataPath().c_str(),
  //               add_card.dataType().c_str(),
  //               add_card.eventType().c_str());
  
  Serial.println("Web add cardUID:"); 
  printResult(add_card); // see addons/RTDBHelper.h
  Serial.println();
  String value = add_card.stringData();


  if (value.length() == 8) {
      Serial.print(EEPROM.read(0)); // Đọc số lượng mã thẻ từ địa chỉ đầu tiên của EEPROM

    add_web = true;
    // Chuyển đổi String thành mảng byte
    byte cardUID[4];
    for (int i = 0; i < 4; i++) {
      cardUID[i] = (byte)strtoul(value.substring(i * 2, i * 2 + 2).c_str(), NULL, 16);
    }
    // Ghi dữ liệu vào EEPROM
    writeID(cardUID);
    add_web = false;
  } else {
    Serial.println("Invalid cardUID format, must be 8 characters");
  }
  //Serial.printf("Received stream payload size: %d (Max. %d)\n\n", add_card.payloadLength(), add_card.maxPayloadLength());
  dataChanged = true;
}

void streamTimeoutCallback_ADD(bool timeout)
{
  if (timeout)
    Serial.println("stream timed out, resuming...\n");

  if (!stream_add.httpConnected())
    Serial.printf("error code: %d, reason: %s\n\n", stream_add.httpCode(), stream_add.errorReason().c_str());
}



void streamCallback_DEL(FirebaseStream del_card)
{
  // Serial.printf("sream path, %s\nevent path, %s\ndata type, %s\nevent type, %s\n\n",
  //               del_card.streamPath().c_str(),
  //               del_card.dataPath().c_str(),
  //               del_card.dataType().c_str(),
  //               del_card.eventType().c_str());
  Serial.println("Web delete cardUID:");              
  printResult(del_card); // see addons/RTDBHelper.h
  String value = del_card.stringData();
  if (value.length() == 8) {
    del_web = true;
    // Chuyển đổi String thành mảng byte
    byte cardUID[4];
    ...
    // Xóa dữ liệu từ EEPROM
    deleteID(cardUID);
    del_web = false;
  } else {
    Serial.println("Invalid cardUID format, must be 8 characters");
  }
  //Serial.printf("Received stream payload size: %d (Max. %d)\n\n", del_card.payloadLength(), del_card.maxPayloadLength());
  dataChanged = true;
}

void streamTimeoutCallback_DEL(bool timeout)
{
  if (timeout)
    Serial.println("stream timed out, resuming...\n");
